...
export default function Home() {
  ...
  const calculateKPI = () => {
    ...
    if (sector === "Groothandel & Distributie") {
      ...
      calc = { marge: marge.toFixed(2) + "%", leverbetrouwbaarheid: leverbetrouwbaarheid.toFixed(2) + "%", stockrotatie: stockrotatie.toFixed(2) };
    } else if (sector === "Andere / Algemeen") {
      const facturatiegraad = (formData.factBinnen30 / formData.factTotaal) * 100;
      calc = { facturatiegraad: facturatiegraad.toFixed(2) + "%", manueleRegistraties: formData.manueleRegistraties, dataBeschikbaarheid: formData.dataBeschikbaarheid + "%" };
    }
    setResults(calc);
    setStep("results");
  };
